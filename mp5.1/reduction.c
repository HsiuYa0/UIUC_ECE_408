...
__global__ void total(float *input, float *output, int len) {
  ...
  //@@ correct index
  __shared__ int partition[BLOCK_SIZE * 2];
  int bx = blockIdx.x; 
  int tx = threadIdx.x;
  
  for (int k = 0; k < 2; k++) {
    int inputIndex = tx + (bx*(BLOCK_SIZE*2)) + (k*BLOCK_SIZE);
    if(inputIndex < len){
      partition[tx + (k*BLOCK_SIZE)] = input[inputIndex];
    }else{
      partition[tx + (k*BLOCK_SIZE)] = 0;
    }
    __syncthreads();
  }
  ...
  for(int stride = 1 ; stride <= BLOCK_SIZE; stride *=2){
    if (tx % stride == 0) {
      partition[rSx] += partition[rSx + stride];
    }
    __syncthreads();
  }
  
  if (tx == 0) {
    output[bx] = partition[0];
  }
}
